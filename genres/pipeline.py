...
from .models import EnrichedBook, BookInfo
...
class BookDataOrchestrator:
    ...
    def enrich_books(self, books: List[BookInfo], max_books: Optional[int] = None) -> List[EnrichedBook]:
        """
        ...
        self.logger.info(f"Starting enrichment pipeline for {len(books_to_process)} books")
    ...
    def display_pipeline_summary(self) -> None:
        ...
        openlib_success = sum(1 for book in self.enriched_books if 
                             book.openlib_edition_response or book.openlib_work_response)
        final_success = sum(1 for book in self.enriched_books if len(book.final_genres) > 0)
        
        print("\n" + "=" * 70)
        print("📊 BOOK DATA ENRICHMENT PIPELINE SUMMARY")
        print("=" * 70)
        print(f"Total books processed: {total}")
        print(f"Google Books success: {google_success} ({google_success/total*100:.1f}%)")
        print(f"Open Library success: {openlib_success} ({openlib_success/total*100:.1f}%)")
        print(f"Final enrichment success: {final_success} ({final_success/total*100:.1f}%)")
        
        # Genre statistics
        total_genres = [len(book.final_genres) for book in self.enriched_books]
        if total_genres:
            print(f"\nGenre Statistics:")
            print(f"  Average genres per book: {sum(total_genres)/len(total_genres):.1f}")
    ...
    def display_source_comparison(self) -> None:
        ...
        google_counts = [len(book.processed_google_genres) for book in self.enriched_books]
        openlib_counts = [len(book.processed_openlib_genres) for book in self.enriched_books]
        ...
        print(f"\nOpen Library:")
        print(f"  Average subjects per book: {sum(openlib_counts)/len(openlib_counts):.1f}")
        ...
        for book in self.enriched_books:
            if book.processed_google_genres and book.processed_openlib_genres:
                overlap = analyze_genre_overlap(book.processed_google_genres, book.processed_openlib_genres)
    ...
    def display_detailed_book_analysis(self, book_title: str) -> None:
        ...
        matching_books = [book for book in self.enriched_books 
    ...
    def save_results(self, 
                    csv_file: str = "genres/results/enriched_books.csv",
                    json_file: str = "genres/results/enrichment_report.json") -> None:
        ...
        if self.enriched_books:
            ...
            report = {
                ...
                "pipeline_stats": {
                    "google_success_rate": sum(1 for b in self.enriched_books if b.google_response) / len(self.enriched_books) * 100,
                    "openlib_success_rate": sum(1 for b in self.enriched_books if b.openlib_edition_response or b.openlib_work_response) / len(self.enriched_books) * 100,
                    "final_success_rate": sum(1 for b in self.enriched_books if len(b.final_genres) > 0) / len(self.enriched_books) * 100,
                },
                "genre_stats": {
                    "avg_final_genres": sum(len(b.final_genres) for b in self.enriched_books) / len(self.enriched_books),
                    "avg_google_genres": sum(len(b.processed_google_genres) for b in self.enriched_books) / len(self.enriched_books),
                    "avg_openlib_genres": sum(len(b.processed_openlib_genres) for b in self.enriched_books) / len(self.enriched_books),
                }
            }
